...
def process_snapshot(snapshot, index):
    try:
        ...
        print(f"提取的iframe源地址: {src}")

        # 打开iframe源地址
        driver = page.open_url_with_edge(src, headless=True, wait_time=0)
        ...
        filename = f"kuaishou_agreement_{date_str}.txt"
...
# 1️⃣ 设置目标协议页面 URL
url = "https://www.kuaishou.com/about/policy"  # ← 你可以换成别的平台链接
user_agent = "Mozilla/5.0"


# 2️⃣ 在桌面新建保存文件夹
desktop_path = "./test/"
save_folder = os.path.join(desktop_path, "kuaishou_User_Agreements")
