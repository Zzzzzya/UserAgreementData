...
def extract_wayback_prefix(url):
    """
    从Wayback Machine URL中提取前缀部分，截止到第二个http之前
    
    参数:
    url (str): Wayback Machine的完整URL
    
    返回:
    str: Wayback Machine的前缀部分，格式如https://web.archive.org/web/时间戳/
         如果URL格式不正确，返回None
    
    示例:
    >>> url = "https://web.archive.org/web/20170814114048/http://map.amap.com:80/doc/serviceitem.html"
    >>> extract_wayback_prefix(url)
    'https://web.archive.org/web/20170814114048/'
    """
    # 找到第一个http出现的位置
...
def process_snapshot(snapshot, index):
    try:
        ...
        driver.quit()
        # print(res)
        src_url = page.find_iframe_src(html_content, iframe_id="service_item")
        print(prefix+src_url)

        driver = page.open_url_with_edge(prefix+src_url, headless=True, wait_time=0)
        # page.try_method_click(driver, "服务协议")
        html_content = driver.page_source
        driver.quit()
        ...
        filename = f"gaode_agreement_{date_str}.txt"
...
# 1️⃣ 设置目标协议页面 URL
url = "http://map.amap.com/doc/serviceitem.html"  # ← 你可以换成别的平台链接
user_agent = "Mozilla/5.0"


# 2️⃣ 在桌面新建保存文件夹
desktop_path = "./test/"
save_folder = os.path.join(desktop_path, "gaode_User_Agreements")
