...
# 1️⃣ 设置目标协议页面 URL
url = "http://update.unisdk.163.com/html/latest_v5.html"  # 腾讯新闻用户协议
user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"

# 2️⃣ 在桌面新建保存文件夹 - 修改为腾讯新闻文件夹
desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
save_folder = os.path.join(desktop_path, "wyyx_User_Agreements")
...
for i, snapshot in enumerate(tqdm(filtered_snapshots, desc="抓取协议中（每周一条）")):
    ...
    try:
        ...
        # 更新为腾讯新闻协议文件名
        filename = f"wyyx_agreement_{date_str}.txt"
